...
/*
    发送文字信息
*/
$msg = '{"toUser": "san.zhang","msgType":"text","text":{"content": "it"}}';
list($errcode, $encrypt) = $ydapi->EncryptMsg($msg);
if ($errcode === 0) {
    list($errcode, $rsp) = $ydapi->Send(API_SEND_MSG, $encrypt);
    if ($errcode === 0) {
        echo '发送文字信息成功<br/>';
    } else {
        echo "发送文字信息失败 errcode: $errcode, errmsg: $rsp<br/>";
        exit();
    }
}
...
if ($errcode === 0) {
    ...
    if ($errcode === 0) {
        ...
        echo "发送外链信息失败 errcode: $errcode, errmsg: $rsp<br/>";
        exit();
    }
}
